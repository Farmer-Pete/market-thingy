...
class Statements:
    ...
    income_statements = [
        re.compile(
            r'^((unaudited|condensed|consolidated)\s?)*statements? of (income|operations|earnings|comprehensive loss|comprehensive income).*',
            re.IGNORECASE)]
    balance_sheets = [
        re.compile(r'^((unaudited|condensed|consolidated)\s?)*balance sheets?.*', re.IGNORECASE),
        re.compile(r'^((condensed|consolidated)\s?)+ statement of financial position.*', re.IGNORECASE)]
    cash_flows = [
        re.compile(r'^((unaudited|condensed|consolidated)\s?)*statements? of cash flows.*', re.IGNORECASE)]
...
class Filing:
    ...
    def _get_financial_data(self, statement_regexps, get_all):
        '''
        Returns financial data used for processing 10-Q and 10-K documents
        '''
        financial_data = []

        for names in self._get_statement(statement_regexps):
            short_name = names[0]
            filename = names[1]
    ...
    def _get_statement(self, statement_regexps):
        '''
        ...
        if FILING_SUMMARY_FILE in self.documents:
            ...
            short_names = list(self.get_short_names(filing_summary_xml))

            if not short_names:
                print('No short names found for any documents')

            for short_name in short_names:
                if any(regexp.match(short_name) for regexp in statement_regexps):
        ...
        if len(statement_names) == 0:
            print('No financial documents could be found. Likely need to update constants in edgar.filing.Statements.')
            print(f'statement_regeps = {statement_regexps}')
            breakpoint()

        return statement_names

    def get_html_file_name(self, filing_summary_xml, report_short_name):
        '''
    ...
    def get_short_names(self, filing_summary_xml):
        for report in filing_summary_xml.find_all('report'):
            short_name = report.find('shortname')
            if short_name is not None:
                yield short_name.get_text()
